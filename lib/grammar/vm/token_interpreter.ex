defmodule Grammar.VM.TokenInterpreter do
  ...
  """
  @spec run(tuple(), non_neg_integer(), tuple()) ::
          {:ok, non_neg_integer(), %{optional(atom()) => raw_capture() | [raw_capture()]}} | :fail
  def run(instructions, entry, stream), do: run_from(instructions, entry, stream, 0)

  @doc "Like `run/3`, but starts at `start_pos` instead of 0 -- for matching one of *several* top-level occurrences in the same stream (`Grammar.VM.run_sequence/4`)."
  @spec run_from(tuple(), non_neg_integer(), tuple(), non_neg_integer()) ::
          {:ok, non_neg_integer(), %{optional(atom()) => raw_capture() | [raw_capture()]}} | :fail
  def run_from(instructions, entry, stream, start_pos) do
    loop(instructions, entry, start_pos, stream, [0], [], [{:done, nil}], fresh_frame(), nil)
  end

  defp fresh_frame, do: %{pending: [], captures: %{}}

  defp loop(instrs, ip, pos, stream, ref_stack, backtrack, calls, frame, last_result) do
    case elem(instrs, ip) do
      {:token, name} ->
        case at(stream, pos) do
          %Token{name: ^name} ->
            loop(instrs, ip + 1, pos + 1, stream, ref_stack, backtrack, calls, frame, last_result)

          _ ->
            fail(instrs, backtrack, stream)
        end
      ...
      {:cap_end, name} ->
        ...
        loop(instrs, ip + 1, pos, stream, ref_stack, backtrack, calls, new_frame, last_result)
      ...
      {:indent_exit} ->
        ...
        loop(instrs, ip + 1, pos, stream, rest_ref, backtrack, calls, frame, last_result)
      ...
      {:samecol_check} ->
        case at(stream, pos) do
          ...
          %Token{column: col} ->
            ...
            if col == ref do
              loop(instrs, ip + 1, pos, stream, ref_stack, backtrack, calls, frame, last_result)
            else
              fail(instrs, backtrack, stream)
            end
        end
      ...
      {:jmp, target} ->
        loop(instrs, target, pos, stream, ref_stack, backtrack, calls, frame, last_result)
      ...
      {:commit, target} ->
        [_ | rest_bt] = backtrack
        loop(instrs, target, pos, stream, ref_stack, rest_bt, calls, frame, last_result)

      {:test_progress, loop_target} ->
        [{saved_pos, _, _, _, _} | rest_bt] = backtrack

        if pos == saved_pos do
          loop(instrs, ip + 1, pos, stream, ref_stack, rest_bt, calls, frame, last_result)
        else
          loop(instrs, loop_target, pos, stream, ref_stack, rest_bt, calls, frame, last_result)
        end
      ...
      {:fail_twice} ->
        case backtrack do
          [_ | rest_bt] -> fail(instrs, rest_bt, stream)
          [] -> :fail
        end
    end
  end
  ...
  defp build_raw_capture(:token, ref_name, stream, start_pos, _end_pos, _last_result) do
    {:token, ref_name, elem(stream, start_pos).text}
  end
  ...
  defp fail(_instrs, [], _stream), do: :fail

  defp fail(instrs, [{saved_pos, saved_ref, saved_frame, saved_calls, target} | rest_bt], stream) do
    loop(instrs, target, saved_pos, stream, saved_ref, rest_bt, saved_calls, saved_frame, nil)
  end
end
